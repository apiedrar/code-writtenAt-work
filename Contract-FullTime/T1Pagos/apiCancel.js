...
Examples:
  node ${path.basename(__filename)} ./data/cancellations.csv
  node ${path.basename(
    __filename
  )} ./data/cancellations.csv ./output/results.xlsx
  node ${path.basename(__filename)} "/path/to/Merc-Cancel-20250724.csv"
  node ${path.basename(
    __filename
  )} "/path/to/input.csv" "/path/to/CancelResults.xlsx"

Required CSV Columns:
  - idCargo: The cargo ID for the cancellation request
  - montoTotal: The cancellation amount

Environment Variables Required:
  - api_token: Your API authorization token
  - APIcancelBaseURL: The base URL for the cancellation API
  `);
}
...
async function main() {
  ...
  if (args.length >= 2) {
    ...
    outputExcelFile = path.join(
      inputDir,
      `CancelResults-${inputBasename}-${timestamp}.xlsx`
    );
  }
  ...
  if (!baseUrl) {
    console.error(
      "Error: API base URL not found. Make sure APIcancelBaseURL is set in your .env file"
    );
    process.exit(1);
  }
