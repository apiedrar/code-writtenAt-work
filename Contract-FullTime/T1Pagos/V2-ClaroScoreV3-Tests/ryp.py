...
CSV_ERROR_PATH = os.path.join(BASE_PATH, f"errors_logging/errors_ryp_{timestamp}.csv")
...
RYP_PROFILES = {
    "RyP": lambda payload: payload["purchase"].update({"total_amount": round(random.uniform(500, 10000), 2)})
}
...
def generate_payload(profile, email, phone, token, bin_value=None):
    ...
    if profile in RYP_PROFILES:
        RYP_PROFILES[profile](payload)
...
class VelocityTestUser(HttpUser):
    ...
    def _get_random_profile(self):
        return random.choice(list(RYP_PROFILES.keys()))

    def _get_headers(self):
        return {
            "x-api-key": f"{ryp_token}",
    ...
    def test_token_velocity(self):
        ...
        self._send_request(payload, "Velocity-Token-RyP")
    ...
    def test_email_velocity(self):
        ...
        bin_value = random.choice(DATA_CACHE["velocity_test"]["bines"]) if DATA_CACHE["velocity_test"]["bines"] else "411111"
        payload = generate_payload(profile, email, phone, token, bin_value)
        self._send_request(payload, "Velocity-Email-RyP")
    ...
    def test_phone_velocity(self):
        ...
        bin_value = random.choice(DATA_CACHE["velocity_test"]["bines"]) if DATA_CACHE["velocity_test"]["bines"] else "411111"
        payload = generate_payload(profile, email, phone, token, bin_value)
        self._send_request(payload, "Velocity-Phone-RyP")
    ...
    def test_velocity_rules(self):
        ...
        bin_value = random.choice(DATA_CACHE["velocity_test"]["bines"]) if DATA_CACHE["velocity_test"]["bines"] else "411111"
        payload = generate_payload(profile, email, phone, token, bin_value)
        self._send_request(payload, "Velocity-Rules-RyP")

@events.test_start.add_listener
def on_test_start(environment, **kwargs):
    logger.info("✅ Tests for RyP-Telcel in ClaroScore API: Initializing")

@events.test_stop.add_listener
def on_test_stop(environment, **kwargs):
    logger.info("🛑 Tests for RyP-Telcel in ClaroScore API: Terminated")
