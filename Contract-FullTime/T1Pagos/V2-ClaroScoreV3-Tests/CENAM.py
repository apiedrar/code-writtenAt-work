...
load_dotenv()
cenam_token = os.getenv("CENAM_Token")
...
CSV_ERROR_PATH = os.path.join(BASE_PATH, f"errors_logging/errors_cenam_{timestamp}.csv")
...
CENAM_PROFILES = {
    "CENAM": lambda payload: payload["purchase"].update({"total_amount": round(random.uniform(500, 10000), 2)})
}
...
def generate_payload(profile, email, phone, token, bin_value=None):
    ...
    if profile in CENAM_PROFILES:
        CENAM_PROFILES[profile](payload)
...
class VelocityTestUser(HttpUser):
    ...
    def _get_random_profile(self):
        return random.choice(list(CENAM_PROFILES.keys()))

    def _get_headers(self):
        return {
            "x-api-key": f'{cenam_token}',
    ...
    def test_token_velocity(self):
        ...
        self._send_request(payload, "Velocity-Token-CENAM")
    ...
    def test_email_velocity(self):
        ...
        bin_value = random.choice(DATA_CACHE["velocity_test"]["bins"]) if DATA_CACHE["velocity_test"]["bins"] else "411111"
        payload = generate_payload(profile, email, phone, token, bin_value)
        self._send_request(payload, "Velocity-Email-CENAM")
    ...
    def test_phone_velocity(self):
        ...
        bin_value = random.choice(DATA_CACHE["velocity_test"]["bins"]) if DATA_CACHE["velocity_test"]["bins"] else "411111"
        payload = generate_payload(profile, email, phone, token, bin_value)
        self._send_request(payload, "Velocity-Phone-CENAM")
    ...
    def test_velocity_rules(self):
        ...
        bin_value = random.choice(DATA_CACHE["velocity_test"]["bins"]) if DATA_CACHE["velocity_test"]["bins"] else "411111"
        payload = generate_payload(profile, email, phone, token, bin_value)
        self._send_request(payload, "Velocity-Rules-CENAM")

@events.test_start.add_listener
def on_test_start(environment, **kwargs):
    logger.info("✅ Tests for CENAM in ClaroScore API: Initializing")

@events.test_stop.add_listener
def on_test_stop(environment, **kwargs):
    logger.info("🛑 Tests for CENAM in ClaroScore API: Terminated")
